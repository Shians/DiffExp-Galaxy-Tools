...
#    ###
#       10.cpmReq            -Float specifying cpm requirement
#       11.sampleReq         -Integer specifying cpm requirement
#       12.fdrThresh        -Float specifying the FDR requirement
#       13.lfcThresh        -Float specifying the log-fold-change requirement
#       14.workMode         -String specifying exact test or GLM usage
#       15.htmlPath         -String specifying path to HTML file
#       16.folderPath       -STring specifying path to folder for output
#    IF workMode is "classic" (exact test)
#       17.pairData[2]      -String specifying first group for exact test
#       18.pairData[1]      -String specifying second group for exact test
#    ###
#    IF workMode is "glm"
#       17.contrastData     -String specifying contrasts to be made
#       18.roastOpt         -String specifying usage of gene-wise tests
#       19.hairpinReq       -String specifying hairpin requirement for gene-
#                            wise test
#       20.selectOpt        -String specifying type of selection for barcode
#                            plots
#       21.selectVals       -String specifying members selected for barcode
#                            plots
#    ###
#
...
fdrThresh <- as.numeric(argv[12])
lfcThresh <- as.numeric(argv[13])
workMode <- as.character(argv[14])
htmlPath <- as.character(argv[15])
folderPath <- as.character(argv[16])

if (workMode=="classic") {
  pairData <- character()
  pairData[2] <- as.character(argv[17])
  pairData[1] <- as.character(argv[18])
} else if (workMode=="glm") {
  contrastData <- as.character(argv[17])
  roastOpt <- as.character(argv[18])
  hairpinReq <- as.numeric(argv[19])
  selectOpt <- as.character(argv[20])
  selectVals <- as.character(argv[21])
}
